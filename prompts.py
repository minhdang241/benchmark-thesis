...
PROMPTS = [
    ...
    #     "n_predict": 50,
    #     "text": (
    #         "Collaborative inference between small language models on edge and large language models on cloud infrastructure has emerged as a compelling approach for enabling intelligent services across resource-constrained environments. Instead of relying exclusively on centralized cloud inference, which introduces latency, privacy risks, and poor adaptation to local contexts, or compressing large models to fit on individual edge devices, which introduces loss in accuracy, collaborative inference exploits the combined strength of both approaches. The cloud-hosted LLMs provide strong generalization and zero-shot transfer across tasks, while the edge-hosted SLMs provide fast response times, data locality, and personalization. The intrinsic difference between the hardware foundations enables hierarchical task allocation. For example, SLMs can execute real-time perception, privacy-sensitive preprocessing, and intent filtering, and then offload complex generation and reasoning to LLMs only when necessary. For commodity IoT hardware such as Raspberry Pi clusters, the collaborative inference provides a practical balance. Individual edge devices lack the memory and computational power to execute large models on their own. However, when combined, the resources can be sufficient. Classify the approach described above into one of these categories: (a) single-device edge inference, (b) cloud-only inference, (c) edge-cloud collaborative inference, or (d) distributed edge-only inference. State only the letter and category name in one sentence."
    #     ),
    # },
    {
        "id": "P03",
